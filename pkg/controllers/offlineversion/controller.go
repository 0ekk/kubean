...
func (c *Controller) mergeManifestsStatus(localartifactset *localartifactsetv1alpha1.LocalArtifactSet, manifests []*manifestv1alpha1.Manifest) error {
	klog.Infof("Merge manifest status for %v", manifests)
...
func (c *Controller) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	klog.Infof("Reconcile localartifactset %s", localartifactset.Name)
	sprayRelease, ok := localartifactset.ObjectMeta.Labels[constants.KeySprayRelease]
	if !ok {
		klog.Infof("No labelSelector %s found in %s", constants.KeySprayRelease, localartifactset.Name)
		return controllerruntime.Result{}, nil
	}
	...
	if err := c.mergeManifestsStatus(localartifactset, manifests); err != nil {
		return controllerruntime.Result{RequeueAfter: Loop}, nil
	}
